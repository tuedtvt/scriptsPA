...
def getflightschedule(date):    
    ...
    regexchuyenbay = r"\d*\s{2}\w{3}/\w{3}\s{4}\d{4}/\d{4}"
    chuyenbay= re.findall(regexchuyenbay,allflight)              
    return chuyenbay
...
if __name__ == "__main__":
    result = ""
    sdate = date(2021, 5, 7)   # start date
    edate = date(2021, 5, 7)   # end date
    delta = edate - sdate
    ...
    print(result)
    signoutsabre(Token_Key)
    text_file = open("Outputallvnbl.txt", "w")
